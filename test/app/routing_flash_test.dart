...
void main() {
  testWidgets('로딩 중이어도 온보딩 완료 초기값이 true 면 온보딩 화면이 안 뜬다', (tester) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          ...
          biometricLockEnabledProvider.overrideWith(
            (ref) => Completer<bool>().future,
          ),
  ...
  testWidgets('락 사용자는 로딩 중에도 초기값 true 로 잠금 화면으로 라우팅된다', (tester) async {
    final db = AppDatabase.forTesting(NativeDatabase.memory());
    await SettingsRepository(db).setSetting('onboarding_done', 'true');
      ...
      ProviderScope(
        overrides: [
          ...
          biometricServiceProvider.overrideWithValue(
            FakeBiometricService()..authResult = false,
          ),
          onboardingDoneProvider.overrideWith(
            (ref) => Completer<bool>().future,
          ),
