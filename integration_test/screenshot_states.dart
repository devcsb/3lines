...
void main() {
  ...
  Future<void> seed(EntryRepository entries, {int days = 30}) async {
    const a1 = ['따뜻한 커피 한 잔', '가족과의 저녁 식사', '맑은 가을 날씨', '오래된 좋은 책', '저녁 산책', '친구의 응원'];
    ...
    for (var i = 0; i < days; i++) {
      ...
      await entries.saveEntry(DailyEntry(
        date: du.dateToString(d), emotion: 1 + ((i * 3 + 2) % 5),
        prompt1: '오늘 감사한 작은 것 하나는?', answer1: a1[i % a1.length],
        prompt2: '오늘 받아들인 것은?', answer2: a2[i % a2.length],
        prompt3: '내일의 작은 다짐은?', answer3: a3[i % a3.length],
      ));
    }
  }
